...
async function testFlow() {
  try {
    ...
    // Get an active extinguisher
    console.log('\n[2] Fetching active extinguishers...');
    const extRes = await axios.get(`${GATEWAY_URL}/api/extinguishers?limit=5`, { headers: userHeaders });
    const extinguisher = extRes.data.data[0];
    if (!extinguisher) {
      throw new Error('No extinguishers found for testing.');
    }
    ...
    const usersRes = await axios.get(`${GATEWAY_URL}/api/users?role=INSPECTOR`, { headers: adminHeaders });
    ...
    const inspToken = inspLogin.data.data.token;
    ...
    maintReportRes.data.data.recentActivities.slice(0, 3).forEach(act => {
